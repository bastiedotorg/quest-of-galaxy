...
class ShowRegisterPage extends AbstractLoginPage
{
    ...
    function send()
    {
        ...
        if ($config->game_disable || $config->registration_closed) {
        ...
        if ($config->referral_active && !empty($referralID)) {
        ...
        $verifyURL = 'index.php?page=verify&i=' . $validationID . '&k=' . $validationKey;

        if (!$config->validate_mail_address || !empty($externalAuthUID)) {
