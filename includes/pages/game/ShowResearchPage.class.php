...
class ShowResearchPage extends AbstractGamePage
{
    ...
    private function CancelBuildingFromQueue()
    {
        ...
        if ($PLANET['id'] == $USER['b_tech_planet']) {
            ...
            if (isset($costResources[RESS_METAL])) {
                $sql .= $resource[RESS_METAL] . " = " . $resource[RESS_METAL] . " + :" . $resource[RESS_METAL] . ", ";
                $params[':' . $resource[RESS_METAL]] = $costResources[RESS_METAL];
            }
            if (isset($costResources[RESS_CRYSTAL])) {
                $sql .= $resource[RESS_CRYSTAL] . " = " . $resource[RESS_CRYSTAL] . " + :" . $resource[RESS_CRYSTAL] . ", ";
                $params[':' . $resource[RESS_CRYSTAL]] = $costResources[RESS_CRYSTAL];
            }
            if (isset($costResources[RESS_DEUTERIUM])) {
                $sql .= $resource[RESS_DEUTERIUM] . " = " . $resource[RESS_DEUTERIUM] . " + :" . $resource[RESS_DEUTERIUM] . ", ";
                $params[':' . $resource[RESS_DEUTERIUM]] = $costResources[RESS_DEUTERIUM];
            }
    ...
    private function isQueueFull($ActualCount)
    {
        return Config::get()->max_queue_tech != 0 && Config::get()->max_queue_tech <= $ActualCount;
    }
    ...
    private function AddBuildingToQueue($elementId, $AddMode = true)
    {
        ...
        if (!in_array($elementId, $reslist['tech'])
            || !BuildFunctions::isTechnologyAccessible($USER, $PLANET, $elementId)
        ...
        if ($this->isQueueFull($ActualCount)) {
            return false;
        }
    ...
    public function show()
    {
        ...
        foreach ($reslist['tech'] as $elementId) {
            if (!BuildFunctions::isTechnologyAccessible($USER, $PLANET, $elementId))
                continue;
        ...
        $this->assign(array(
            ...
            'IsFullQueue' => $this->isQueueFull(count($TechQueue)),
            'Queue' => $TechQueue,
        ));
