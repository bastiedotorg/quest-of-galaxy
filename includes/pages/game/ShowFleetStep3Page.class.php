...
class ShowFleetStep3Page extends AbstractGamePage
{
    ...
    public function show()
    {
        ...
        if ($targetMission != MISSION_RECYCLE && IsVacationMode($targetPlayerData)) {
            $this->printMessage($LNG['fl_target_exists'], array(array(
                'label' => $LNG['sys_back'],
                'url' => 'game.php?page=fleetStep1'
            )));
        ...
        if (in_array($targetMission, [MISSION_ATTACK, MISSION_TOGETHER, MISSION_DESTROY])) {
        ...
        if (in_array($targetMission, [MISSION_ATTACK, MISSION_TOGETHER, MISSION_HOLD, MISSION_SPY, MISSION_DESTROY])) {
        ...
        if (in_array($targetMission, [MISSION_HOLD, MISSION_DARKMATTER, MISSION_EXPEDITION, MISSION_TRADE])) {
        ...
        $fleetResource = array(
            RESS_METAL => min($TransportMetal, floor($PLANET[$resource[RESS_METAL]])),
            RESS_CRYSTAL => min($TransportCrystal, floor($PLANET[$resource[RESS_CRYSTAL]])),
            RESS_DEUTERIUM => min($TransportDeuterium, floor($PLANET[$resource[RESS_DEUTERIUM]] - $consumption)),
        );
        ...
        $PLANET[$resource[RESS_METAL]] -= $fleetResource[RESS_METAL];
        $PLANET[$resource[RESS_CRYSTAL]] -= $fleetResource[RESS_CRYSTAL];
        $PLANET[$resource[RESS_DEUTERIUM]] -= $fleetResource[RESS_DEUTERIUM] + $consumption;
