...
class StatBuilder
{
    ...
    private function GetTechnoPoints($USER)
    {
        ...
        foreach ($reslist['tech'] as $Techno) {
            ...
            for ($i = 1; $i <= $USER[$resource[$Techno]]; $i++) {
                $TechPoints += ($pricelist[$Techno]['cost'][RESS_METAL] + $pricelist[$Techno]['cost'][RESS_CRYSTAL] + $pricelist[$Techno]['cost'][RESS_DEUTERIUM]) * pow($pricelist[$Techno]['factor'], $i - 1);
            }
    ...
    private function GetBuildPoints($PLANET)
    {
        ...
        foreach ($reslist['build'] as $Build) {
            ...
            for ($i = 1; $i <= $PLANET[$resource[$Build]]; $i++) {
                $BuildPoints += ($pricelist[$Build]['cost'][RESS_METAL] + $pricelist[$Build]['cost'][RESS_CRYSTAL] + $pricelist[$Build]['cost'][RESS_DEUTERIUM]) * pow($pricelist[$Build]['factor'], $i - 1);
            }
    ...
    private function GetDefensePoints($USER)
    {
        ...
        foreach (array_merge($reslist['defense'], $reslist['missile']) as $Defense) {
            ...
            $Units = $pricelist[$Defense]['cost'][RESS_METAL] + $pricelist[$Defense]['cost'][RESS_CRYSTAL] + $pricelist[$Defense]['cost'][RESS_DEUTERIUM];
    ...
    private function GetFleetPoints($USER)
    {
        ...
        foreach ($reslist['fleet'] as $Fleet) {
            ...
            $Units = $pricelist[$Fleet]['cost'][RESS_METAL] + $pricelist[$Fleet]['cost'][RESS_CRYSTAL] + $pricelist[$Fleet]['cost'][RESS_DEUTERIUM];
