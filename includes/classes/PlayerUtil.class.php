...
class PlayerUtil
{
    ...
    static public function isInactive($data)
    {
        return (isset($data['onlinetime']) && $data['onlinetime'] < TIMESTAMP - INACTIVE);
    }
    ...
    static public function checkPosition($universe, $galaxy, $system, $position)
    {
        ...
        return !(1 > $galaxy
            || 1 > $system
            || 1 > $position
            || $config->maximum_galaxies < $galaxy
            || $config->maximum_systems < $system
            || $config->maximum_planets < $position);
    }
    ...
    static public function createPlayer($universe, $userName, $userPassword, $userMail, $userLanguage = NULL, $galaxy = NULL, $system = NULL, $position = NULL, $name = NULL, $authlevel = 0, $userIpAddress = NULL)
    {
        ...
        if (isset($universe, $galaxy, $system, $position)) {
            ...
            $galaxy = $config->latest_galaxy_position;
            $system = $config->latest_system_position;
            $planet = $config->latest_planet_position;

            if ($galaxy > $config->maximum_galaxies) {
                $galaxy = 1;
            }

            if ($system > $config->maximum_systems) {
                $system = 1;
            }

            do {
                $position = mt_rand(round($config->maximum_planets * 0.2), round($config->maximum_planets * 0.8));
                if ($planet < 3) {
                    $planet += 1;
                } else {
                    if ($system >= $config->maximum_systems) {
            ...
            $config->latest_galaxy_position = $galaxy;
            $config->latest_system_position = $system;
            $config->latest_planet_position = $planet;
        }
    ...
    static public function createPlanet($galaxy, $system, $position, $universe, $userId, $name = NULL, $isHome = false, $authlevel = 0)
    {
        ...
        $dataIndex = (int)ceil($position / ($config->maximum_planets / count($planetData)));
        $maxTemperature = $planetData[$dataIndex]['temp'];
        $minTemperature = $maxTemperature - 40;

        if ($isHome) {
            $maxFields = $config->planet_initial_fields;
        } else {
    ...
    static public function maxPlanetCount($USER)
    {
        ...
        $planetPerTech = $config->max_planets_tech;
        $planetPerBonus = $config->max_planets_officer;
    ...
    static public function getPositionRequirement($position, $USER): int
    {
        ...
        switch ($position) {
            case 1:
            case ($config->maximum_planets):
                return 8;
                break;
            case 2:
            case ($config->maximum_planets - 1):
                return 6;
                break;
            case 3:
            case ($config->maximum_planets - 2):
                return 4;
                break;
            default:
                return 1;
